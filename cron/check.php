...
} else {//-- Boo. Service is down, let's add it for Amazon, otherwise, just mark it offline and send the alert, if set like that
	$addToAmazonQueue = ServiceManagement::shouldAddToAmazonQueue($result->id);

	if ($addToAmazonQueue) {
		ServiceManagement::addToAmazonQueue($result->id);
		echo "\nADDED TO AMAZON QUEUE\n";
	} else {//-- Mark the service offline
		$fields = array(
			'updated' => $now,
			'next_update' => HelperGuru::getNextUpdate($result->mmi),
			'extra' => json_encode($check->msg),
